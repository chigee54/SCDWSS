...
def generate_embeddings_train_predict(corpora, corpora_predict, outputpath):
    ...
    # clf_two = RandomForestClassifier(n_estimators=250, criterion='gini', min_samples_leaf=1, min_samples_split=2, random_state=10)
    ...
    for document_path in corpora:
        ...
        for paragraph_index, paragraph in enumerate(paragraphs):
            ...
            if previous_para_embeddings is not None:
                ...
                # two_para_embeddings = torch.cat([previous_para_embeddings, current_para_embeddings], dim=0)
                if torch.cuda.is_available():
                    two_para_embeddings = two_para_embeddings.cpu()
                two_paragraphs_embeddings.append(two_para_embeddings.numpy())
                # if two_paragraphs_embeddings is not None:
                #     two_paragraphs_embeddings = torch.cat([two_paragraphs_embeddings, two_para_embeddings], dim=0)
                # else:
                #     two_paragraphs_embeddings = two_para_embeddings
        ...
        # two_paragraphs_embeddings_list.append(two_paragraphs_embeddings)
        # two_paragraphs_embeddings_list = torch.stack(two_paragraphs_embeddings_list, dim=0)
        ...
        # bool_id = '{}{}\t'.format(share_id, str(np.isnan(two_paragraphs_embeddings.numpy()).any()))
        ...
        # two_paragraphs_label_list.append(two_paragraphs_label)
        # two_paragraphs_embeddings_list.append(two_paragraphs_embeddings)
        # two_paragraphs_embeddings_list = torch.stack(two_paragraphs_embeddings_list, dim=0)
    ...
    # if torch.cuda.is_available():
    #     two_paragraphs_embeddings_list = two_paragraphs_embeddings_list.cpu()
    # 检验是否有NaN值，有的话就补0
    ...
    # two_paragraphs_embeddings_list = [t.numpy() for t in two_paragraphs_embeddings_list]
    # two_paragraphs_label_list = [np.array(t) for t in two_paragraphs_label_list]
    # two_paragraphs_label_list = list(chain.from_iterable(two_paragraphs_label_list))
    # two_paragraphs_embeddings_list = list(chain.from_iterable(two_paragraphs_embeddings_list))
    # two_paragraphs_embeddings_list = torch.stack(two_paragraphs_embeddings_list, dim=0)
    clf_one = clf_one.fit(document_embeddings_list, document_label_list)
    # with open('tensor', 'w', encoding='utf-8') as fff:
    #     fff.write(str(two_paragraphs_embeddings_list.numpy()))
    # print(shape(two_paragraphs_embeddings))
    # print(shape(two_paragraphs_label_list))
    # clf_two = clf_two.fit(two_paragraphs_embeddings, two_paragraphs_label_list)
    ...
    def generate_embeddings_predict(corpora, outputpath):
        for document_path in corpora:
            ...
            for paragraph_index, paragraph in enumerate(paragraphs):
                ...
                #     two_para_embeddings = two_para_embeddings.unsqueeze(0)
                #     if torch.cuda.is_available():
                #         two_para_embeddings = two_para_embeddings.cpu()
                #     two_para_label = clf_two.predict(two_para_embeddings)
                #     two_para_label_list.append(int(two_para_label))
                #
                # previous_para_embeddings = current_para_embeddings
                # previous_para_length = current_para_length
                # pure_para_length_list.append(current_para_length)
            ...
            # try:
            # task2_pre_label = []
            # for i in range(len(two_paragraphs_embeddings.numpy())):
            #     two_paragraphs_labels = clf_two.predict(two_paragraphs_embeddings[0])
            #     task2_pre_label.append(int(two_paragraphs_labels))
            # except:
...
def compute_avg_two_para(para_length_a, para_length_b, para_embedding_a, para_embedding_b):
    ...
    cp_para_embeddings = (para_embedding_a / para_length_a) + (para_embedding_b / para_length_b)
...
if __name__ == '__main__':
    input_train = 'demo_data20/train2'
    input_val = 'demo_data20/validation1'
    # input_train = '../pan21/train'
    # input_val = '../pan21/validation'
    # input_train = '../pan20-style-change-detection/train1'
    # input_val = '../pan20-style-change-detection/validation1'
    output_dir = 'task1_result'
    ...
    write_output(os.path.join(output_dir, "evaluation.txt"), 'task1_score', task1_results)
    # write_output(os.path.join(output_dir, "evaluation.txt"), 'task2_score', task2_results)